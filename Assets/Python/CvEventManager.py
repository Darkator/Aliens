...
####	
	...
	def onBeginGameTurn(self, argsList):
		...
		if iGameTurn == 45:
			...
			self.pPolandID= 7
			self.pEnglandTeam = gc.getTeam(gc.getPlayer(self.pEnglandID).getTeam())
			self.pEnglandTeam.addTeam(gc.getPlayer(self.pPolandID).getTeam())
			...
			###declare war begin###
			iTypeDeclaringCiv = gc.getInfoTypeForString("CIVILIZATION_RUSSIA")
			iTypeVictim = gc.getInfoTypeForString("CIVILIZATION_WW2_FINLAND")
			iDeclaringPlayer = -1
			iVictimPlayer = -1
			iMaxCiv = gc.getMAX_PLAYERS()
			for iCivs in range(iMaxCiv):
			...
			###declare war begin###
			iTypeDeclaringCiv = gc.getInfoTypeForString("CIVILIZATION_RUSSIA")
			iTypeVictim = gc.getInfoTypeForString("CIVILIZATION_WW2_BALTIC")
			iDeclaringPlayer = -1
			iVictimPlayer = -1
			iMaxCiv = gc.getMAX_PLAYERS()
			for iCivs in range(iMaxCiv):
		...
		if iGameTurn == 53:
			...
			self.pGermanyTeam = gc.getTeam(gc.getPlayer(iDeclaringPlayer).getTeam())
		...
		if iGameTurn == 63:
			...
			self.pGermanyTeam.addTeam(gc.getPlayer(self.pBulgariaID).getTeam())
			self.pYugoslaviaID = 28
			self.pEnglandTeam = gc.getTeam(gc.getPlayer(self.pEnglandID).getTeam())
...
## Reinforcement Script ##
	...
	#	###81 - 87####
	#	if iGameTurn > 81:
	#		Research.addSpeed(iPlayer)
	#	####64 - 69###
	#	if iGameTurn > 66:
	#		Research.Axisspeed(iPlayer)
	#		## Tech Resource ##
